...
# from production_line_monitor.util.data_utils import get_finally_process, get_process_sequence_dict, get_code_map_dict
...
def device_handler(msg):
    ...
    # 测试使用
    # get_logger().info('get_finally_process: {}'.format(get_finally_process('product_line_6t90')))
    # get_logger().info('get_code_map_dict: {}'.format(get_code_map_dict('product_line_6t90')))
...
def hxd_handler(msg):
    ...
    for line in line_list:
        ...
        for process in process_list:
            change_process_dict[product_line].append({"code":process["code"], "sequence":process["sequence"]})
    get_logger().info('change_process_dict ~~~~~~~~~~~~~~~~~~~:%s ', change_process_dict)


    # 测试使用
    # get_logger().info('get_process_sequence_dict: {}'.format(get_process_sequence_dict('product_line_6t90')))
...
def get_change_process_dict_list(product_line):
    return [i for i in change_process_dict[product_line]]

def get_change_process_list(product_line):
    return [i["code"] for i in change_process_dict[product_line]]

def get_change_sequence_list(product_line):
    return [i["sequence"] for i in change_process_dict[product_line]]
